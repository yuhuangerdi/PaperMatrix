...
## 4. paper.yaml（当前 schema_version: 11）
...
浏览器上传只在请求期间读取 PDF 元数据和指纹，不把 PDF 写入工作区。由于浏览器不会提供可长期访问的绝对路径，此类记录保存 `source.path: null` 并标记 `unlinked`。v1–v10 记录读取时通过纯函数迁移到 v11 内存模型，后续正常保存时写回 v11；旧版松散结构化摘要、既有条目内容和来源字段保持不变。
...
阶段 3 为阶段 4 准备的可分析条目保存在 `structured_summary.items[]`。阶段 4A 将该投影扩展为包含稳定 `item_id`、固定模板 `section_key`、章节内顺序、来源锚点、来源笔记 revision、固定 `kind`、可选 `display_label`、标题、摘要、结构化属性、`evidence_ids`、自由标签、写作用途和时间戳。证据目录保存在同级 `structured_summary.evidence_catalog[]`。`kind` 用于稳定筛选与跨论文分析，`display_label` 只改变用户看到的条目标签。

每条证据同时有内部 UUID 和用户可读的 `evidence_code`，例如 `E-001`。笔记第 11 节先登记证据，条目正文再以 `证据：E-001` 引用；确认时条目只保存对应 UUID 到 `evidence_ids`，不复制页码、图表和说明。缺少证据允许保存，但状态必须可被矩阵识别为待补证据。
...
论文登记时立即创建 revision 1 的默认笔记。第 0 节不属于分析条目，其字段全部由论文概览中的 `bibliography` 和 `organization` 维护；概览保存后使用笔记 revision、锁和原子替换同步第 0 节，完整文档保存时也恢复这些权威字段，但不重排第 1–11 节。旧记录缺少笔记文件时仍可按 revision 0 的兼容流程首次保存。

结构化笔记始终保留模板第 0–11 节和完整、连续的 Markdown，不为每个条目创建单独文件。固定模板槽位以原标题为边界，即使为空也能在条目模式读取；首次保存实质内容时在标题前加入不影响渲染的 `<!-- papermatrix:item:<uuid> -->` 稳定来源锚点。标题下的段落、列表、键值和整张表格属于同一条目。2.2 代表性文献使用三级父标题作为可扩展分组，每个四级文献标题形成独立条目。Markdown 正文和顺序为权威内容，`paper.yaml` 中的条目是可重建、经确认的检索与分析投影。

完整文档模式和条目模式编辑同一份 Markdown。固定槽位更新按 `template_key` 精确定位原标题，只替换标题下正文；清空时保留模板骨架并移除投影。可扩展条目根据稳定锚点替换或删除完整四级标题块。投影记录来源笔记 revision 及来源片段 SHA-256 指纹；指纹或 revision 不一致时进入候选审阅或待同步状态。正文删除会形成待确认删除项，不会静默删除投影。确认条目的 `is_favorite` 是本地阅读视图的重点收藏状态，保存在同一条目投影中，不写入 Markdown。

新内容优先使用条目模式原生录入：固定项直接填写模板已有标题下的 Markdown 正文，不新建标题；只有 `repeatable` 分组可新增带标题和稳定锚点的独立条目。旧笔记和外部修改仍由候选审阅同步。第 11 节及 `structured_summary.evidence_catalog[]` 共同构成论文级证据目录；条目模式右侧独立展示该目录，新增证据时自动分配未占用的 `E-xxx`，并可同时在当前条目正文写入显式引用。
...
每条证据使用稳定 `evidence_id`，可记录印刷页码、从 0 开始的 PDF 页序号、章节、图号、表号和定位说明。证据不保留条目 ID；笔记条目只能通过正文中显式的 `证据：E-001` 引用建立关联。证据的 `paper_id` 固定为当前论文。该文件是问题清单的权威来源，Excel 不重复维护问题答案。
