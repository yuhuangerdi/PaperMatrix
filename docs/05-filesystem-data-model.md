...
## 4. paper.yaml（当前 schema_version: 7）
...
结构化笔记始终保留模板第 0–12 节和完整、连续的 Markdown，不为每个条目创建单独文件。经确认条目在 Markdown 中具有不影响正常渲染的 `<!-- papermatrix:item:<uuid> -->` 稳定来源锚点；标记统一位于三级或四级语义标题前，标题下的段落、列表、键值和整张表格属于同一条目。Markdown 正文和顺序为权威内容，`paper.yaml` 中的条目是可重建、经确认的检索与分析投影。

完整文档模式和条目模式编辑同一份 Markdown。条目更新根据稳定锚点只替换对应标题区块，保留标记、其他章节、空行和段落顺序。旧版本已经按表格行确认的投影在读取时保持可诊断；用户确认新的整表候选后，服务端移除旧行锚点、合并人工标签/写作用途/证据，并以标题块条目替换旧投影。投影记录来源笔记 revision 及来源片段 SHA-256 指纹；指纹或 revision 不一致时进入候选审阅或待同步状态，只有用户明确确认候选后才更新投影，不能用旧投影覆盖较新的正文。
