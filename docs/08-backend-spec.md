...
论文仓储枚举项目记录时必须逐文件迁移和校验。单个记录失败时返回脱敏的无效记录摘要，继续加载其他有效论文；不得因一条损坏或未来版本记录让整个列表失败，也不得在读取列表时自动覆盖原文件。已发布的文件 Schema 版本必须保留纯函数迁移，或提供显式、可测试且不丢数据的升级流程。

阶段 4A/4B 分别增加个人补充笔记、条目关系和问题归纳仓储。所有 paper/item/scope/problem 引用都必须验证属于当前项目；问题归纳矩阵由权威对象组装，不单独落盘为业务事实。

新建条目关系时服务层验证来源和目标均为当前项目的已确认条目。读取既有关系时不强制目标继续存在，而是组装 `available`、`missing_paper` 或 `missing_item` 诊断；删除条目前的影响查询只读 `item-links.yaml`，用户确认后保留悬空关系，不静默级联删除。

项目文献总矩阵和准备度是请求时从 `papers/*.yaml` 与可选 `scopes.yaml` 组装的只读投影，不单独落盘。方法、实验、局限与证据分别判断，未知或缺失不转成分数排名；集合中缺失论文只返回诊断，不修改范围。
