...
- 条目模式预载第 1–8 节的全部模板填写项（空项也显示），固定项原位填写；代表性文献、挑战和创新点各保留一个占位，可新增或删除独立条目；
...
- 在软件内使用覆盖背景、问题、方法、实验、发现、局限和证据的项目文献总矩阵，按全部论文或分析集合查看，并分项检查分析准备度；
- 建立多个绑定分析集合的问题归纳板，将已确认的论文研究问题映射为领域问题，并在每篇论文下分列维护方法和七态解决程度；
- 使用贡献检查器分别保存判断理由、方法/实验条目、证据、反证和成立条件；桌面双列矩阵与移动端按问题列表使用同一权威数据；
- 检测源 PDF 缺失或变化，重新关联时不移动原文件。

阶段 4A 的分析准备与范围已完成；阶段 4B 已完成领域问题归纳的首个垂直切片，包括归纳板、领域问题映射、论文贡献和方法/解决程度双列矩阵。下一切片将继续方法分类、挑战—方案矩阵和实验可比性。
